...
<script type="text/javascript" src="js/Chart.min.js"></script>
    ...
    <meta charset="UTF-8">
    <title>Admin Panel | Aplikasi Persediaan Pakan & Ternak</title>
  ...
  <body class="hold-transition skin-blue sidebar-collapse sidebar-mini">
    ...
    <script>
        $(document).ready(function () {
          $jsontext =["datachart/hewan.php","datachart/pakan.php","datachart/penjualan.php"]; 
          $target = ["#graphHewan","#graphPakan","#graphPenjualan"];
          $title = ["Stok Hewan","Stok Pakan","Total Penjualan per Bulan"];
          $color = ["#69BE69","#CE794E","#9999FF"];
          $jenis = ["horizontalBar","horizontalBar","bar"];
          for (i=0;i<$target.length;i++){
             showGraph($jsontext[i],$target[i],$title[i],$color[i],$jenis[i]);
          }
          
        });
 
        function showGraph($jsontext,$target,$title,$color,$jenis)
        {            
                $.post($jsontext,
                function (data)
                {
                    console.log(data);
                     var name = [];//["A","B","C","D","E"];
                     var stock = [];//[40,13,10,47,60];
                     //var barcolor = ["#FF0000","#FFFF00","#00FF00","#00FFFF","#0000FF"];

                    for (var i in data) {
                        name.push(data[i].item_name);
                        stock.push(data[i].amount);
                    }
